...
class CreatePatientFollowupsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('patient_followups', function (Blueprint $table) {
            ...
            $table->string('no_years_inhalers')->nullable();
            ...
            $table->integer('fuel1_yr');
            $table->enum('main_fuel2',['Electricity', 'LPG','Gobar gas','Kerosene','Charcoal','Coal/coke','Dung cake','Wood','Crop residues','Others',''])->default('');
            $table->integer('fuel2_yr');

            // $table->foreign('created_by')->references('id')->on('users'); //verbose style
            // $table->foreign('patient_id')->references('id')->on('patients'); // verbose style
